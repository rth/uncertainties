...
# Python < 2.6 does not have math.isinf:
def isinf(x):
    '''
    Similar to the math.isinf() of Python 2.6+.
    '''
    if not isinstance(x, float):
        raise TypeError('a float is required')
    # !!! Maybe set([...]) is faster?
    return x in (float("inf"), float("-inf"))

# Python < 3.6 does not have math.isinfinite:
...
def format_num(nom_val_main, error_main, common_exp,
    ...
    inf), the parts are applied as much as possible to the nominal
    value; this fails with Python < 2.6 if the resulting format is not
